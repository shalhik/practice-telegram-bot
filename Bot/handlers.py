...
async def start(msg: Message):
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Подключиться к ClickUp", callback_data="connect")],
...
async def connect(callback: CallbackQuery):
    ...
    if not spaces:
        await callback.message.answer(
            ...
            "Проверьте:\n"
            "1. API ключ ClickUp установлен в .env файле (CLICKUP_API_KEY)\n"
            "2. TEAM_ID правильный в Bot/config.py\n"
    ...
    await callback.message.edit_text("Выберите Space ClickUp:", reply_markup=markup)
    await callback.answer()
...
async def important(msg: Message):
    tasks = await get_important_tasks(msg.chat.id)
    if not tasks:
        await msg.answer("Важных задач не найдено для ваших подписок.")
        return

    lines = [
        f"{task.get('name', 'Без названия')} — {task.get('url', '')}"
        for task in tasks[:10]
    ]
    await msg.answer("Список важных задач:\n" + "\n".join(lines))
